...
object NetworkDiagnostics {
    ...
    suspend fun fullReport(context: Context): String = withContext(Dispatchers.IO) {
        ...
        sb.appendLine()
        sb.appendLine("🔎 DNS:")
        sb.appendLine("  [система] www.bing.com  → ${resolveDns("www.bing.com")}")
        sb.appendLine("  [система] wallhaven.cc  → ${resolveDns("wallhaven.cc")}")
        sb.appendLine("  [система] picsum.photos → ${resolveDns("picsum.photos")}")
        sb.appendLine("  [DoH]     www.bing.com  → ${DoHResolver.resolve("www.bing.com") ?: "✗ все провайдеры недоступны"}")
        sb.appendLine("  [DoH]     wallhaven.cc  → ${DoHResolver.resolve("wallhaven.cc") ?: "✗ все провайдеры недоступны"}")
        sb.appendLine("  [DoH]     picsum.photos → ${DoHResolver.resolve("picsum.photos") ?: "✗ все провайдеры недоступны"}")
        sb.appendLine()
        ...
        sb.appendLine()
        sb.appendLine("🌐 Wallhaven API:")
        sb.append(checkUrl("https://wallhaven.cc/api/v1/search?sorting=toplist&topRange=1d&atleast=1920x1080&purity=100&categories=110", UA_CHROME))
        sb.appendLine()
        sb.appendLine("🖼️ Wallhaven CDN (w.wallhaven.cc):")
        sb.append(checkUrl("https://w.wallhaven.cc/", UA_CHROME))
        sb.appendLine()
        sb.appendLine("🌐 Picsum:")
        sb.append(checkUrl("https://picsum.photos/v2/list?page=1&limit=1", UA_CHROME))
        sb.appendLine()
        sb.appendLine("🌐 DoH Cloudflare (1.1.1.1):")
        sb.append(checkUrl("https://1.1.1.1/dns-query?name=wallhaven.cc&type=A", UA_OKHTTP))
        sb.appendLine()
        sb.appendLine("🌐 DoH Google (8.8.8.8):")
        sb.append(checkUrl("https://8.8.8.8/resolve?name=wallhaven.cc&type=A", UA_OKHTTP))
        sb.appendLine()
